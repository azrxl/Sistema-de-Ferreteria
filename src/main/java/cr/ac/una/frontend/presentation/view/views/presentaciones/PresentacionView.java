package cr.ac.una.frontend.presentation.view.views.presentaciones;
...
import cr.ac.una.proxy.objects.Presentacion;
...
public class PresentacionView {
    ...
    private void confirmarPedido() {
        ...
        try {
            ...
            carritoTableModel.fireTableDataChanged();
            presentacionTableModel.fireTableDataChanged();

            mostrarFactura(factura);
        } catch (Exception ex) {
            JOptionPane.showMessageDialog(null, "Error al confirmar el pedido: " + ex.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
        }
    }
    ...
    public void mostrarFactura(Factura factura) {
        String message = String.format("""
                ...
                -----------------------------
                Subtotal: $%.2f
                Descuento por unidades (>10 unidades): $%.2f
                Descuento por artículos diferentes (>10 diferentes): $%.2f
                Descuento por monto (> $5000): $%.2f
                -----------------------------
                Total de descuento: $%.2f
                Total a pagar: $%.2f
                """,
                factura.getSubtotal(),
                factura.getDescuentoUnidades(),
                factura.getDescuentoDiferentes(),
                factura.getDescuentoMonto(),
                factura.getTotalDescuento(),
                factura.getTotalFinal());

        JOptionPane.showMessageDialog(null, message, "Factura del Pedido", JOptionPane.INFORMATION_MESSAGE);
    }
    ...
    static class PresentacionTableModel extends AbstractTableModel<Presentacion> {
        ...
        public PresentacionTableModel(List<Presentacion> items, Controller controller) {
            super(new String[]{"Unidad", "Capacidad", "Compra", "Venta", "Cantidad"}, items);
            this.controller = controller;
        }
        ...
        public Object getValueAt(int rowIndex, int columnIndex) {
            ...
            return switch (columnIndex) {
                case 0 -> item.getUnidad(); // Capacidad
                case 1 -> item.getCapacidad(); // Unidad
